import { is, header, versions, log, err, last, values, client } from './utils'

export default function(ripple){
  var resources = ripple._resources()

  // record a new historical value for a resource
  version.record = function({ name, key, value, type }) {
    if (!client) return;
  ...
  version.history = function() {
    ...
    resources
      .time = resources.versions.length-1
  ...
  function version(name, version){
    if (!client) return;
    return arguments.length == 2                 ? resource({ name: name, index: version })
         : arguments.length == 1 && is.str(name) ? resource(name)
         : arguments.length == 1 && is.num(name) ? application(name)
         : arguments.length == 0                 ? application()
         : err('Couldn\'t rollback', name, version)
  }
  ...
  function resource(o) {
    ...
    if (is.str(o)) 
      return is.num(header('version')(resources[o]))
           ? header('version')(resources[o])
           : (versions(resources, o).length-1)
  ...
  function application(time) { 
    ...
    if (!arguments.length) return resources.time
