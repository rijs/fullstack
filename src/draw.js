import { all, err, later, client, resourcify, attr, applyhtml, applycss, is, body, values, header, key, prepend } from './utils'
...
export default function(ripple){
  ...
  return function draw(thing){
    ...
    return !thing                             ? components()
         ...
         : this && this.nodeName              ? invoke(this)
         : this && this.node                  ? invoke(this.node())
         : is.str(thing)                      ? resource(thing)
         : thing.name                         ? resource(thing)
         : thing[0] instanceof MutationRecord ? invoke(thing[0].target)
         : err('Couldn\'t update', thing)
  }
